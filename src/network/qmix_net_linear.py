...
class QMixNet(nn.Module):
    def __init__(self, args):
        ...
        if args.two_hyper_layers:
            ...
            self.hyper_w1 = nn.Linear(args.state_shape, args.n_agents * 1)
            # 经过hyper_w2得到(经验条数, 1)的矩阵
    ...
    def forward(self, q_values, states):  # states的shape为(episode_num, max_episode_len， state_shape)
        ...
        q_values = q_values.reshape(-1, 1, self.args.n_agents)  # (episode_num * max_episode_len, 1, n_agents) = (1920,1,5)  todo actor_sample_times

        states = states.reshape(-1, self.args.state_shape)  # (episode_num * max_episode_len, state_shape)

        w1 = torch.abs(self.hyper_w1(states))  # (1920, 160) #todo
        # w1 = self.hyper_w1(states) # (1920, 160) #todo noabs
        ...
        w1 = w1.view(-1, self.args.n_agents, 1)  # (1920, 5, 1)
        b1 = b1.view(-1, 1, 1)  # (1920, 1, 1)

        q_total=torch.bmm(q_values, w1) + b1
